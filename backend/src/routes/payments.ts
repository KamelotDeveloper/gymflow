  ...
  try {
    ...
    try {
      mpPreference = await createMPPreference({
        ...
        back_urls: {
          success: `${process.env.FRONTEND_URL || 'http://localhost:5173'}/membresia?txn=${txn.id}`,
          failure: `${process.env.FRONTEND_URL || 'http://localhost:5173'}/membresia?error=mp_failure`,
          pending: `${process.env.FRONTEND_URL || 'http://localhost:5173'}/membresia?txn=${txn.id}`,
        },
        auto_return: 'all',
